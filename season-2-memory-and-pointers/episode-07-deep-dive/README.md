...
├── README.md             ← Вы здесь
├── mission.md
├── starter.c             ← Шаблон с malloc/free
├── artifacts/
│   └── dynamic_loader.c  ← Создайте этот файл
├── solution/
...
1. **Изучите** `starter.c` — там структуры и заготовки с malloc/free
2. **Создайте** файл `artifacts/dynamic_loader.c`
3. **Реализуйте** работу с динамической памятью:
   - Чтение заголовка (фиксированный размер)
   - Чтение блоков с `malloc()` под данные переменного размера
   - Построение индекса блоков
   - Вывод содержимого
   - **Обязательно** `free()` для каждого `malloc()`!
4. **Скомпилируйте**: `gcc -Wall -o dynamic_loader artifacts/dynamic_loader.c`
5. **Проверьте утечки**: `valgrind ./dynamic_loader archive.dat`

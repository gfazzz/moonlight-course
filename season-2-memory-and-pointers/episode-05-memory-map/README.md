...
├── artifacts/
│   └── memory_mapper.c ← Создайте этот файл
├── solution/
...
1. **Изучите** `starter.c` — там заготовки функций с TODO
2. **Создайте** файл `artifacts/memory_mapper.c`
3. **Реализуйте** функции для парсинга разных типов данных:
   - `calculate_checksum()` — проверка целостности
   - `parse_string()` — извлечение строк
   - `parse_coordinates()` — извлечение координат (float)
   - `parse_timestamp()` — извлечение времени (uint32_t)
4. **Скомпилируйте**: `gcc -Wall -o memory_mapper artifacts/memory_mapper.c`
5. **Запустите**: `./memory_mapper memory_dump.dat`

...
1. Выделенная память не освобождается → накопление → исчерпание RAM. Пример: `p = malloc(100); p = malloc(200);` — первый блок потерян.
2. `valgrind --leak-check=full ./program` — показывает ВСЕ утечки с точным местом выделения.
3. Проверка malloc на NULL, обнуление после free, RAII (парные create/destroy).
4. Система пытается освободить уже освобождённую память → corruption → краш! Защита: `free(p); p = NULL;`
5. Вместо malloc/free в цикле → выделить ОДИН буфер, переиспользовать → меньше syscalls → быстрее.
6. malloc (выделение) → проверка NULL → использование → free (освобождение) → p = NULL (обнуление).
7. **valgrind** (Linux): детальный анализ, показывает stack traces. **leaks** (macOS): быстрый, показывает количество/размер утечек.
8. Dangling pointer → указывает на освобождённую память. NULL делает его безопасным (краш сразу, а не UB).
9. Ресурс привязан к времени жизни объекта: конструктор выделяет, деструктор освобождает. В C: create/destroy пары.
10. Локальные переменные живут в **Stack** → автоматически освобождаются при выходе из функции!
11. Heap разбит на куски разных размеров → появляются "дыры" → неэффективное использование памяти.
12. Без free → память накапливается → OOM (Out Of Memory) → краш программы/системы!
...
Вы выходите на Красную площадь. Октябрьское солнце. Ветер.

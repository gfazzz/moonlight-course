...
├── README.md              ← Вы здесь
├── mission.md
├── buggy_logger.c         ← Программа с утечками памяти
...
1. **Изучите** `buggy_logger.c` — там утечки памяти (не компилируйте его!)
...
5. **Проверьте**: `valgrind` должен показать "0 bytes in 0 blocks"

> ⚠️ **Важно**: Используйте `starter.c` как справочник с подсказками!

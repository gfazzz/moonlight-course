...
├── artifacts/
│   └── pointer_navigator.c ← Создайте этот файл
├── solution/
...
1. **Изучите** `starter.c` — там структура `Entry` и заготовки функций
2. **Создайте** файл `artifacts/pointer_navigator.c`
3. **Реализуйте** навигацию по указателям:
   - Чтение массива структур из файла
   - Поиск записи по `offset`
   - Обход цепочки по полю `next`
   - Защита от зацикливания
   - Декодирование ASCII-символов
4. **Скомпилируйте**: `gcc -Wall -o pointer_navigator artifacts/pointer_navigator.c`
5. **Запустите**: `./pointer_navigator pointers.dat`

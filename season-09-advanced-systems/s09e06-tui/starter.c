 ...
 *
 * Одна ячейка = один СИМВОЛ, а не один байт: ячейка хранит последовательность
 * байт целиком, поэтому текст внутри экрана не обязан быть ASCII. Символ
 * шириной два знакоместа (иероглиф) занимает две ячейки — вторая пустая.
 * Разбор UTF-8 и таблица ширин — как в s05e08.
 */
#include <stdio.h>
#include <string.h>
...
/* Реализуй.
   Понадобятся две вспомогательные функции, которых нет в списке ниже:
     разбор одной кодовой точки UTF-8 (сколько байт съедено, 0 при ошибке)
     и её ширина в знакоместах (0 для комбинирующих, 2 для иероглифов, иначе 1).
   Обе есть в s05e08 — можно взять оттуда. */
static void clear(Screen *s);
static void put(Screen *s, int x, int y, char c, unsigned char a);   /* с отсечением по границам */
/* text кладёт символ целиком в одну ячейку и двигает курсор на ЕГО ШИРИНУ,
   а не на число байт. Символ шириной 2 занимает вторую ячейку пустой. */

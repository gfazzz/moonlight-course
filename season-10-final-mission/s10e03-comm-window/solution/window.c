...
/* Привести к чистому нулю величину, которая заведомо ниже разрешения вывода.
   Нужно из-за отрицательного нуля: значение вида -3e-17 печатается как
   «-0.0000», а его знак определяется порядком операций и потому меняется
   между платформами и уровнями оптимизации. */
static double snap_zero(double v, double eps) {
    return (v < eps && v > -eps) ? 0.0 : v;
}
...
int main(void) {
    ...
    for (int i = 0; i <= 6; i++) {
        ...
        /* В точке наибольшего сближения радиальная скорость равна нулю точно:
           спутник идёт поперёк луча. Численно там остаётся мусор порядка 1e-17,
           и его ЗНАК зависит от порядка операций — на разных платформах
           печаталось то -0.0000, то 0.0000. Величина ниже разрешения вывода
           физического смысла не имеет, поэтому приводим её к чистому нулю. */
        vr    = snap_zero(vr,    5e-5);                 /* половина последнего разряда %.4f */
        shift = snap_zero(shift, 5e-3);                 /* то же для %.2f            */

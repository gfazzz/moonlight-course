...
```
> LUNA: "Верно! `unsigned char` гарантирует диапазон 0-255 (все возможные байты).
>        Обычный `char` может быть знаковым (-128 до 127) или беззнаковым (зависит от реализации).
>        
>        Если `char` знаковый и ты сохраняешь 0xFF (255), он становится -1. Катастрофа!
>        Виктору нужен был ТОЧНЫЙ диапазон 0-255 для бинарных данных.
>        
>        Профессиональный совет: ВСЕГДА используй `unsigned char` для бинарных данных. Поверь мне."
```
</details>
...
> LUNA: "A) 1 байт. ✓
>        
>        0x42 = 66 в десятичной. Идеально помещается в один байт (диапазон 0-255).
>        Мы сохраняем его в `unsigned char key = 0x42;` — ровно 1 байт.
>        
>        Забавный факт: 0x42 = 'B' в ASCII. Виктор обожал пасхалки."
```
</details>
...
```
> LUNA: "Вывод: Key: 66 (hex: 0x42)
>        
>        Один и тот же байт, два представления:
>        %d   — десятичный формат: 66
>        %02X — шестнадцатеричный формат (2 цифры, верхний регистр): 42
>        
>        Это ОДНО И ТО ЖЕ значение в памяти. Просто разные форматы отображения.
>        Как сказать 'sixty-six' по-английски и 'шестьдесят шесть' по-русски.
>        Одно число, разный язык. Comprende?"
```
</details>

#!/bin/bash
# OPERATION MOONLIGHT — s01e05 автотест
# decoder.c: декод через ФУНКЦИИ xor_crypt()/checksum() в одном файле.

set -u
cd "$(dirname "$0")" || exit 2

ART="../artifacts/decoder.c"
FALLBACK="../solution/decoder.c"
EXPECTED="expected.txt"
BIN="$(mktemp -u /tmp/moonlight_s01e05.XXXX)"
OUT="$(mktemp /tmp/moonlight_out.XXXX)"

if [ -f "$ART" ]; then SRC="$ART"; WHO="artifacts/decoder.c";
else SRC="$FALLBACK"; WHO="solution/decoder.c (fallback)"; fi
echo "Источник: $WHO"

echo "Компиляция..."
if ! gcc -Wall -Wextra -std=c11 -o "$BIN" "$SRC"; then
    echo "FAIL: не компилируется."; exit 1
fi
echo "OK: собралось."

"$BIN" > "$OUT" 2>&1 || { echo "FAIL: падение при запуске."; cat "$OUT"; exit 1; }

if diff -u "$EXPECTED" "$OUT"; then
    echo; echo "PASS: логика в функциях, без копипаста. Дальше — s01e06 (модули)."; rc=0
else
    echo; echo "FAIL: вывод не совпал с expected.txt (см. diff)."; rc=1
fi
rm -f "$BIN" "$OUT"
exit $rc

...
**Сюжет:** Анализ торговых манипуляций организации на крипто-биржах

**Технологии:**
- Order Book структура (bids/asks)
- Price-Time Priority matching
- Sorting для HFT (High-Frequency Trading)
- Market manipulation detection (pump & dump, spoofing)

**Задачи:**
1. **orderbook.c** — Реализация order book
2. **matching_engine.c** — Order matching engine
3. **anomaly_detector.c** — Детектирование манипуляций рынком

**Артефакт:** `trade_analyzer` для анализа торговых данных

**Метафоры:**
- Order Book = доска объявлений (покупка/продажа)
- Spread = разница между ценой покупки и продажи
- Liquidity = легкость купли-продажи без изменения цены
- Market order = купить/продать СЕЙЧАС по любой цене

---

### Episode 16: "Financial Data Structures"
**Сюжет:** Создание risk management системы для портфеля MOONLIGHT

**Технологии:**
- Linked Lists для streaming tick data
- Stacks для transaction history (undo/redo)
- Queues для order processing pipeline
- Financial metrics: VaR, Sharpe ratio, volatility

**Задачи:**
1. **tick_stream.c** — Linked list для market data
2. **transaction_stack.c** — Stack с undo для операций
3. **risk_calculator.c** — Вычисление VaR и Sharpe ratio

**Артефакт:** `risk_engine` для портфельного анализа

**Метафоры:**
- Linked List = вагоны поезда (каждый знает следующий)
- Stack = стопка тарелок (LIFO - Last In First Out)
- Queue = очередь в магазине (FIFO - First In First Out)
- VaR = максимальный убыток с 95% уверенностью
- Sharpe ratio = доходность / риск

**🎯 Milestone:** Выявлены финансовые махинации организации, портфель MOONLIGHT защищен

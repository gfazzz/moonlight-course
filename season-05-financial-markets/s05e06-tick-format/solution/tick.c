...
/* Та же запись с полями по убыванию размера: дыры исчезают сами. */
typedef struct {
    uint32_t ts;
    int32_t  price;
    uint32_t qty;
...
static int32_t  unzigzag(uint32_t v){ return (int32_t)((v >> 1) ^ (~(v & 1) + 1)); }
...
static size_t encode_delta(uint8_t *out, const tick_t *t, int n) {
    size_t k = 0;
    put_u32(out + k, t[0].ts);    k += 4;      /* опорная точка целиком */
    put_i32(out + k, t[0].price); k += 4;

    for (int i = 1; i < n; i++) {
        out[k++] = (uint8_t)(t[i].ts - t[i - 1].ts);          /* шаг 1..5 сек */
        uint32_t z = zigzag(t[i].price - t[i - 1].price);
        out[k++] = (uint8_t)z;                                /* шаг цены     */
    }
    return k;
}
...
static int decode_delta(const uint8_t *in, tick_t *t, int n) {
    size_t k = 0;
    t[0].ts    = get_u32(in + k); k += 4;
    t[0].price = get_i32(in + k); k += 4;

    for (int i = 1; i < n; i++) {
        t[i].ts    = t[i - 1].ts + in[k++];
        t[i].price = t[i - 1].price + unzigzag(in[k++]);
    }
    return 1;
}

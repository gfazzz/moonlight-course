...
# money.c сводит выписку со счётом и показывает, куда девается недостача.
# Данных на входе нет: выписка зашита в исходник, поток сделок порождается ЛКГ.

set -u
cd "$(dirname "$0")" || exit 2

ART="../artifacts/money.c"
FALLBACK="../solution/money.c"
EXPECTED="expected.txt"
BIN="$(mktemp -u /tmp/moonlight_s05e02.XXXX)"
OUT="$(mktemp /tmp/moonlight_out.XXXX)"
OUT2="$(mktemp /tmp/moonlight_out2.XXXX)"

if [ -f "$ART" ]; then SRC="$ART"; WHO="artifacts/money.c";
else SRC="$FALLBACK"; WHO="solution/money.c (fallback)"; fi
echo "Источник: $WHO"
...
if ! diff -u "$OUT" "$OUT2"; then
    echo; echo "FAIL: вывод изменился при -O2 — значит, деньги считаются в double."
    rm -f "$BIN" "$OUT" "$OUT2"; exit 1
fi

echo; echo "PASS: расхождение найдено и объяснено. Дальше — s05e03 (разбор настоящего CSV)."
rm -f "$BIN" "$OUT" "$OUT2"
exit $rc

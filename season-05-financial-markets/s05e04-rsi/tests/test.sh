#!/bin/bash
# OPERATION MOONLIGHT — s05e04 автотест
# rsi.c: RSI по Уайлдеру + зоны перекупленности/перепроданности. Детерминирован.

set -u
cd "$(dirname "$0")" || exit 2

ART="../artifacts/rsi.c"
FALLBACK="../solution/rsi.c"
EXPECTED="expected.txt"
BIN="$(mktemp -u /tmp/moonlight_s05e04.XXXX)"
OUT="$(mktemp /tmp/moonlight_out.XXXX)"

if [ -f "$ART" ]; then SRC="$ART"; WHO="artifacts/rsi.c";
else SRC="$FALLBACK"; WHO="solution/rsi.c (fallback)"; fi
echo "Источник: $WHO"

echo "Компиляция..."
if ! gcc -Wall -Wextra -std=c11 -o "$BIN" "$SRC"; then
    echo "FAIL: не компилируется."; exit 1
fi
echo "OK: собралось."

"$BIN" > "$OUT" 2>&1 || { echo "FAIL: падение при запуске."; cat "$OUT"; exit 1; }

if diff -u "$EXPECTED" "$OUT"; then
    echo; echo "PASS: RSI и зоны посчитаны. Дальше — s05e05 (бэктест)."; rc=0
else
    echo; echo "FAIL: вывод не совпал с expected.txt (см. diff)."; rc=1
fi
rm -f "$BIN" "$OUT"
exit $rc

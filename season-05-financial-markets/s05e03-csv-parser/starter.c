 ...
 * Каркас: скопируй в artifacts/csv.c и заполни TODO.
 *
 *     cp starter.c artifacts/csv.c
 *     make test
 *
 * Заполнять нужно переходы автомата в csv_parse и две функции накопителя.
 * Отчёт, наивный разбор и печать уже готовы: серия про автомат, а не про
 * форматирование.
 *
...
/* TODO 1. Добавить байт в текущее поле.
   Если места в буфере не осталось — выставить r->overflow и выйти,
   НЕ записывая байт. Молчаливая обрезка записи хуже её потери:
   потерянную видно в отчёте, обрезанная выглядит как настоящая.  */
static void row_putc(row_t *r, char c) {
    (void)r; (void)c;
}

/* TODO 2. Завершить текущее поле: дописать в буфер завершающий ноль,
   запомнить начало следующего поля в r->field[] и увеличить r->nfield.
   Вызывается и на разделителе, и в конце записи.
   Ловушка: считать поля по числу запятых нельзя — у записи из пяти
   полей запятых четыре. Считай завершения, а не разделители.   */
...
static int csv_parse(FILE *f) {
    ...
    (void)row_putc;   /* строка уйдёт, как только появится первый вызов */

    while ((c = fgetc(f)) != EOF) {
        off++;
        switch (state) {

        /* TODO 3. Начало поля: ещё неизвестно, в кавычках оно или нет.
             '"'  — поле в кавычках: отметить r.had_quote и уйти в S_QUOTED
             ','  — поле пустое: завершить его, остаться в S_START
             '\n' — запись кончилась: завершить поле, вызвать report_row,
                    затем row_reset, запомнить row_off = off, pending = 0
             '\r' — это голова CRLF, разделитель, а не данные: пропустить
             иначе — байт данных: записать и уйти в S_PLAIN            */
        case S_START:
            pending = 1;
            break;

        /* TODO 4. Поле без кавычек. Запятая и перевод строки здесь —
           разделители. '\r' — хвост CRLF, пропускается.             */
        case S_PLAIN:
            break;

        /* TODO 5. Внутри кавычек особых символов нет: запятая и перевод
           строки — это ДАННЫЕ, их надо записывать в поле. Отмечай при
           этом r.had_comma и r.had_newline — они нужны отчёту.
           Единственный особый байт здесь — сама кавычка: она переводит
           автомат в S_AFTER_QUOTE, где и решится, что она значила.   */
        case S_QUOTED:
            break;

        /* TODO 6. Кавычка внутри кавычек. Смысл выясняется по следующему
           байту:
             '"'  — это была удвоенная кавычка: записать ОДНУ и вернуться
                    в S_QUOTED
             ','  — поле закончилось: завершить, уйти в S_START
             '\n' — запись закончилась: как в TODO 3
             '\r' — хвост CRLF
             иначе — мусор после закрывающей кавычки; строгий разбор счёл
                    бы это ошибкой, мы записываем байт и уходим в S_PLAIN */
        case S_AFTER_QUOTE:
            break;
        }
    }

...
# ma.c: SMA/EMA и crossover. Детерминирован.

set -u
cd "$(dirname "$0")" || exit 2

ART="../artifacts/ma.c"
FALLBACK="../solution/ma.c"
EXPECTED="expected.txt"
BIN="$(mktemp -u /tmp/moonlight_s05e03.XXXX)"
OUT="$(mktemp /tmp/moonlight_out.XXXX)"

if [ -f "$ART" ]; then SRC="$ART"; WHO="artifacts/ma.c";
else SRC="$FALLBACK"; WHO="solution/ma.c (fallback)"; fi
echo "Источник: $WHO"
...
if diff -u "$EXPECTED" "$OUT"; then
    echo; echo "PASS: SMA/EMA и пересечения посчитаны. Дальше — s05e04 (RSI)."; rc=0
else
